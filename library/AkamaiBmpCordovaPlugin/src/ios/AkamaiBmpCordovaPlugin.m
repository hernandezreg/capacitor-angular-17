...
@interface AkamaiBmpCordovaPlugin : CDVPlugin <CYFChallengeActionDelegate, CYFSDKInitDelegate> {
}
...
- (void)initializeSDKWithPow:(CDVInvokedUrlCommand *)command {
	NSString *url = [command.arguments objectAtIndex:0];

	[CYFMonitor configureSDKWithPow:url];
	/**	posting notification so that the listeners will start capturing events	*/
	[[NSNotificationCenter defaultCenter] postNotificationName:@"CYF_BMP_SDK_INITIALIZED"
														object:nil
													  userInfo:nil];
...
- (void)notifyInitStatus:(int)status message:(NSString *)message {
	NSMutableDictionary *response = [NSMutableDictionary dictionary];
	[response setObject:[NSNumber numberWithInt:status] forKey:@"status"];
	if (message) {
		[response setObject:message forKey:@"message"];
...
- (void)initSDKCallback:(CDVInvokedUrlCommand *)command {
	NSDictionary *params = [command.arguments objectAtIndex:0];
		self.callbackId = command.callbackId;
        [CYFMonitor setSDKInitCallBack:(self)];
        CDVPluginResult *pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_NO_RESULT];
		[pluginResult setKeepCallback:[NSNumber numberWithBool:YES]];
		[self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
...
- (void)onSDKInitFailure:(NSString *)message {
      NSLog(@"onSDKInitFailure");
      [self notifyInitStatus:-1 message:message];
}
- (void)onSDKInitSuccess {
     NSLog(@"onSDKInitSuccess");
     [self notifyInitStatus:1 message:nil];
}
