...
/*
 New Dci Methods
*/
exports.initializeSDK = function (baseUrl) {
    exec(null, null, 'AkamaiBmpCordovaPlugin', 'initializeSDK', [baseUrl]);
};

exports.initializeSDKWithPow = function (baseUrl) {
    exec(null, null, 'AkamaiBmpCordovaPlugin', 'initializeSDKWithPow', [baseUrl]);
};
...
exports.setSDKInitCallBack = function (params) {
	...
	let ccaObject = {
		...
		execSuccess: function(response) {
			if (response.status == 1 && this.actionSuccessCallback) {
				this.actionSuccessCallback(response);
			} else if (response.status == -1 && this.actionFailureCallback) {
				this.actionFailureCallback(response);
			}
		...
		execFailure: function(response) {
			console.error("setSDKInitCallBack Failure: "+response);
		}
	};
    console.error("setSDKInitCallBack:");
	exec(function(param) {
			ccaObject.execSuccess(param);
		},
		function(param) {
